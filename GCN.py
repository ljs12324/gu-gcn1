...
class GCN(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes,dropout1=0.5):
        super(GCN, self).__init__()
        self.conv1 = GraphConv(in_dim, hidden_dim)
        self.conv2 = GraphConv(hidden_dim, hidden_dim)
        self.conv3 = GraphConv(hidden_dim, hidden_dim)
        # self.gru = nn.GRU(input_size=hidden_dim, hidden_size=8)
        # self.classify = nn.Linear(hidden_dim, n_classes)
        self.classify = nn.Sequential(nn.Dropout(p=dropout1), nn.Linear(hidden_dim, n_classes))    #, nn.Tanh()
    def forward(self, g):
        # print(g)
...
def Test(model, flag):
    ...
    tnr = TN / (TN + FP)
    L = np.concatenate((test_Y.reshape((len(test_Y), 1)), Y_pr.reshape((len(Y_pr), 1))), axis=1)
    np.savetxt('/home/ran/Desktop/openrainbow/openrainbow/GCN/data/ls-gcn-1.txt', L,
               fmt='%d %.3f')
    print(
