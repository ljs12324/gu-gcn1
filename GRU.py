...
class GCN(nn.Module):
    def __init__(self, in_dim, hidden_dim, n_classes, dropout1 = 0.7):
        ...
        #self.GRU1 = nn.GRU(input_size=hidden_dim,hidden_size=hidden_dim,batch_first=True)
...
def Test(Model, flag):
    ...
    tnr = TN / (TN + FP)
    L = np.concatenate((test_Y.reshape((len(test_Y), 1)), Y_pr.reshape((len(Y_pr), 1))), axis=1)
    np.savetxt('/home/ran/Desktop/openrainbow/openrainbow/GCN/data/GRU-gu-gcn.txt', L,
               fmt='%d %.3f')
    print(
